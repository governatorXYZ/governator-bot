...
const creator = new SlashCreator({
    applicationID: process.env.DISCORD_BOT_APPLICATION_ID ? process.env.DISCORD_BOT_APPLICATION_ID : '',
...
function initializeSlashCreateEvents(): void {
    ...
    eventFiles.forEach(file => {
        ...
        try {
            if (event.once) {
                creator.once(event.name, (...args: any) => event.execute(...args, client));
            } else {
                creator.on(event.name, (...args: any) => event.execute(...args, client));
            }
            logger.debug(`Registered event ${event.name}`);
        } catch (e: any) {
